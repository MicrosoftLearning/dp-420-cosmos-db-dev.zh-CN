...
1. 选择“+ 创建”按钮添加保管库，并使用以下设置填写保管库，将所有其余设置保留为其默认值，然后选择以创建保管库：
    ...
    | **区域** | 选择任何可用区域 |
    | **访问配置/权限模型** | *保管库访问策略* |
    | **访问配置/访问策略** | *选择当前用户名复选框* |

    > &#128221; 请注意，在生产环境中，最有可能选择 RBAC 控件而不是保管库访问策略，并且管理员很可能向你分配适当的 RBAC 角色，以限制你的密钥保管库。
...
1. 打开 **Visual Studio Code**。  选择“文件”->“打开文件夹”，一直浏览到 28-key-vault 文件夹，以打开 28-key-vault 文件夹 。
    ...
    > &#128221; 请注意，你应该只能在资源管理器树中看到 28-key-vault 文件夹及其文件和子文件夹 。 如果可以看到之前克隆的整个 GitHub 存储库，请 ***关闭 Visual Studio Code*** 并将其直接重新打开到 **28-key-vault** 文件夹。  如果该目录不是项目根目录，则 Web 应用将无法正常工作，因此请确保只能在 **资源管理器** 树中看到 **28-key-vault** 文件夹及其文件和子文件夹。
...
> &#128221; 下面的大部分步骤将在 Visual Studio 屏幕中上部的命令面板 (**CTRL+SHIFT+P**) 中运行。 

1. 在 Visual Studio Code 中，打开命令面板，搜索“Azure 应用服务: 创建新的 Web 应用…(高级)”

1. 选择“登录到 Azure...”******。此选项将打开一个 Web 浏览器窗口，请按照登录流程操作，并在完成后关闭浏览器，然后返回到 Visual Studio Code。
...
1. 在“主体”选项卡的搜索框中，输入你为应用服务提供的名称，从列表中选择它，然后选择“下一步”。
